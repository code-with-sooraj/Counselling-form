<?php
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $query = "select * from student where c_id='$id';";
        $result = mysqli_query($conn,$query);
        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $photo = base64_encode($row['photo']);
            $twelth = base64_encode($row['twelth']);
            $tenth = base64_encode($row['tenth']);
        } else {
            echo "<script>alert('No records found')
            location.href = 'check.php'</script>";
            die();
        }
        mysqli_close($conn);
    }
...
<body>
    <div class="container">
        <div class="content">
            ...
            <div class="upper">
                <div class="uleft">
                    <div class="field"><label>Name: </label><input type="text" value="<?php echo $row['f_name']; ?>" readonly></div>
...
<script>
    ...
    function viewCertificate(type, id) {
        window.open('show.php?type=' + type + '&id=' + id, '_blank');
    }
</script>
</html>